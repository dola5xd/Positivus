...
function TeamCard({ data }) {
  const { id, avatar, name, position, info } = data;
  return (
    <div className="team-card">
      <button className="absolute right-[50px] top-[35px] Tablet:top-auto">
        {Icon}
      </button>
      <div className="flex items-end gap-3">
        <motion.img
          src={avatar}
          alt="avatar"
          variants={{ anime: { rotate: "1turn" }, init: { rotate: 0 } }}
          initial="init"
          animate="anime"
          transition={{
            delay: 0.1 * id,
        ...
        <div>
          ...
          <p className="font-medium text-[1rem]">{position}</p>
        </div>
      </div>
      <hr className="bg-brand_2 w-full h-1" />
      <p className="font-medium text-[1.1rem]">{info}</p>
    </div>
  );
}
